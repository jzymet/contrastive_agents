...
class MathDataset:
    ...
    def check_final_answer(self, predicted: float, ground_truth: float, tolerance: float = 0.01) -> bool:
        try:
...
class GSM8KDataset(MathDataset):
    ...
    def _generate_synthetic_problems(self) -> List[Dict]:
        ...
        templates = [
            {
                "template": "{name} has {n1} apples. {name2} gives {name} {n2} more apples. How many apples does {name} have now?",
                "solution_template": "{n1} + {n2} = {answer}",
                "compute": lambda n1, n2: n1 + n2
            },
            {
                "template": "{name} buys {n1} items at ${n2} each. What is the total cost?",
                "solution_template": "{n1} × {n2} = {answer}",
                "compute": lambda n1, n2: n1 * n2
            },
            {
                "template": "{name} has ${n1}. {name} spends ${n2}. How much money does {name} have left?",
                "solution_template": "{n1} - {n2} = {answer}",
                "compute": lambda n1, n2: n1 - n2
            },
            {
                "template": "{name} wants to split {n1} cookies equally among {n2} friends. How many cookies does each friend get?",
                "solution_template": "{n1} ÷ {n2} = {answer}",
                "compute": lambda n1, n2: n1 // n2 if n2 != 0 else 0
            },
            {
                "template": "{name} reads {n1} pages per day. How many pages will {name} read in {n2} days?",
                "solution_template": "{n1} × {n2} = {answer}",
                "compute": lambda n1, n2: n1 * n2
            },
            {
                "template": "A store has {n1} shirts. They sell {n2} shirts. How many shirts are left?",
                "solution_template": "{n1} - {n2} = {answer}",
                "compute": lambda n1, n2: n1 - n2
            },
            {
                "template": "{name} has {n1} stickers. {name} gets {n2} more stickers from {name2}. How many stickers does {name} have in total?",
                "solution_template": "{n1} + {n2} = {answer}",
                "compute": lambda n1, n2: n1 + n2
            },
            {
                "template": "There are {n1} students in a class. Each student needs {n2} pencils. How many pencils are needed in total?",
                "solution_template": "{n1} × {n2} = {answer}",
                "compute": lambda n1, n2: n1 * n2
            },
        ]
        ...
        for i in range(self.n_problems):
            ...
            question = template_data["template"].format(
                name=name, name2=name2, n1=n1, n2=n2
            )
            
            solution = template_data["solution_template"].format(
    ...
    def generate_step_candidates(self, problem: Dict, current_state: Dict, n_candidates: int = 50) -> List[str]:
        ...
        if len(numbers) >= 2:
            ...
            candidates.extend([
                ...
                f"Calculate {a} - {b} = {a - b}",
                f"Calculate {a} * {b} = {a * b}",
                f"Calculate {a} / {b} = {a / b:.2f}" if b != 0 else f"Calculate {a} / {b} = undefined",
        ...
        while len(candidates) < n_candidates:
            ...
            candidates.append(f"Calculate {rand_a} {op} {rand_b} = {result:.2f}")
...
class MATH500Dataset(MathDataset):
    ...
    def _generate_harder_problems(self) -> List[Dict]:
        ...
        subjects = ["Algebra", "Geometry", "Number Theory", "Probability", "Counting"]
        ...
        for i in range(self.n_problems):
            ...
            if subject == "Algebra":
                ...
                total = 2 ** n
                favorable = int(np.math.comb(n, k))
                ...
                answer = int(np.math.factorial(n))
    ...
    def generate_step_candidates(self, problem: Dict, current_state: Dict, n_candidates: int = 100) -> List[str]:
        candidates = []
